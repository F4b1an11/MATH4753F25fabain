...
#' @param sigma1 true population standard deviation sigm used to generate the random samples
#' @param mean1 ture population mean used to generate the random samples
#' @param iter number of monte carlo simulations to perfom
#' @param ymax upper limit of the y axis for the histogram to control scaling
#' @param legend_pos location for the legend on the plot
#' @param ...
#'
#' @return A list with components:
#' \itemize{
#'   \item `w` – Numeric vector of simulated t statistics (length = `iter`).
#'   \item `summary` – Summary statistics for `w`.
#'   \item `sd` – Standard deviation of `w`.
#'   \item `fun` – Character label `"T"`.
#' }
#' @export
#'
#' @examples
